...
class FeatureGenerator:
    ...
    def write_features(self, wordlist):
        ...
        for word1 in words:
            for word2 in words:
                ...
                if id1 == id2:
                    continue
                hyper_example = self.get_hyper_features(word1, word2, id1, id2)
                hf.write(hyper_example + '\n')
                if int(id1) < int(id2):
                    cohyp_example = self.get_cohyp_features(word1, word2, id1, id2)
                    cf.write(cohyp_example + '\n')
        hf.close()
        cf.close()

    def get_hyper_features(self, word1, word2, id1, id2):
        hyper_example = id1 + '-' + id2
        if id2 in self.broad_to_narrow[id1]:
            hyper_example += '\t' + '1'
        else:
            hyper_example += '\t' + '0'
        hyper_example += self.get_common_features(word1, word2)
        return hyper_example

    def get_cohyp_features(self, word1, word2, id1, id2):
        cohyp_example = id1 + '-' + id2
        if id2 in self.broad_to_narrow[self.narrow_to_broad[id1]]:
            cohyp_example += '\t' + '1'
        else:
            cohyp_example += '\t' + '0'
        cohyp_example += self.get_common_features(word1, word2)
    ...
    def get_common_features(self, word1, word2):
        ...
        example += '\t' + ontf.context_subsumption(v1, v2)
...
def main():
    ...
    fg.write_features('/home/hans/workspace/ontoligual/preprocess/term_freqs_22.tsv')
